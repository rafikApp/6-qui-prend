...
int carteValide(int nombre,Joueur *joueur)
{
    //Card carte;
...
Table *initialiserTable() {
    ...
    //table->nombreDeRanges = nombreDeRanges;
    Carte carte;
    carte.number = 0;
...
int main() {
    ...
    { 
                ...
                scanf("%d", &number);
                while(carteValide(number,joueurReçu)==0)
                {
                    printf("Choisissez une carte valide s'il vous plaît :\n");
                    scanf("%d", &number);
                }
                printf("\n");
                joueurReçu->carteJouet = rechercherCarte(number,joueurReçu);
